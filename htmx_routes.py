...
htmx_page = Blueprint('htmx_page', __name__, static_folder='static', template_folder='templates')


@htmx_page.route("/home", methods=["GET"])
def home():
    books = db.session.query(Book, Author).filter(Book.author_id == Author.author_id).all()
    return render_template("htmx_page.html", books=books)


@htmx_page.route("/submit", methods=["POST"])
def submit():
...
@htmx_page.route("/delete/<int:id>", methods=["DELETE"])
def delete_book(id):
...
@htmx_page.route("/get-edit-form/<int:id>", methods=["GET"])
...
@htmx_page.route("/get-book-row/<int:id>", methods=["GET"])
def get_book_row(id):
    book = Book.query.get(id)
    author = Author.query.get(book.author_id)
...
@htmx_page.route("/update/<int:id>", methods=["PUT"])
def update_book(id):
